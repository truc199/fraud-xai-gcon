...
Dữ liệu thô từ 6 bảng được nối (JOIN) và làm giàu bằng các cửa sổ trượt thời gian (Rolling Windows) 1h, 3h, 24h, 48h, 7 ngày, 30 ngày. Tổng cộng **28 đặc trưng số học** được sinh ra cho mỗi giao dịch (bao gồm 7 đặc trưng nâng cao mới phát hiện gian lận 2026).
...
|---|---|---|
| **Tier 1 — High-Speed Bypass** | Rule-based: Bỏ qua giao dịch nhỏ (<500K VND) với tần suất thấp và chuỗi hành vi bình thường | 1,400 / 5,000 (28%) được bypass an toàn trong ~0.94 ms |
| **Tier 2 — ML Classification** | XGBoost + Elkan-Noto trên 28 features | 3,600 giao dịch mơ hồ được đánh giá, 121 bị gắn cờ (3.36%) trong ~37.90 ms |
| **Tier 3 — xAI Explanation** | TreeSHAP + SHAP Interactions + Counterfactual Recourse | 121 thẻ cảnh báo với lý giải đầy đủ được sinh ra trong ~9.08 giây |
...
Hệ thống sử dụng **28 đặc trưng** chia thành 8 nhóm chức năng. Mỗi đặc trưng dưới đây được mô tả chi tiết từ nguồn dữ liệu, công thức tính toán (measurement), vai trò trong mô hình (biến độc lập/kiểm soát), cho đến lý do thực tiễn chống gian lận.
...
| Rank | Feature | Alerts | % |
|---|---|---|---|
| 1 | TRANS_AMOUNT_Z_SCORE | 87 | 71.90% |
| 2 | BALANCE_COVERAGE_RATIO | 80 | 66.11% |
| 3 | TRANS_AMOUNT_VS_30D_AVG_RATIO | 64 | 52.89% |
| 4 | DAYS_AMOUNT_COMBINED | 24 | 19.83% |
| 5 | TRANS_HOUR | 24 | 19.83% |
| 6 | VELOCITY_RATIO_AMOUNT_7D_VS_30D | 23 | 19.00% |
| 7 | HOURS_SINCE_SEC_EVENT | 15 | 12.39% |
| 8 | SEC_AMOUNT_COMBINED | 13 | 10.74% |
| 9 | HIST_BIOMETRIC_RATIO | 13 | 10.74% |
| 10 | DAYS_SINCE_LAST_TRANS | 7 | 5.78% |

*(Lưu ý: Đặc trưng mới `BENFORD_DEV` xếp hạng 11 với 6 alerts - 4.95%)*
...
| Giai đoạn | Thời gian |
|---|---|
| Tier 1 (Rule Bypass) | ~0.94 ms |
| Tier 2 (ML Classification) | ~37.90 ms |
| Tier 3 (xAI Explanation) | ~9,086.47 ms |
| **Tổng** | **~9,125.31 ms** |
...
|---|---|
| Sau drift, **CÓ** EWC (λ=50) | 0.1410 |
| Sau drift, **KHÔNG** EWC (λ=0) | 0.1690 |
