...
class MLPipeline:
    ...
    def run_training_pipeline(self, limit: Optional[int] = None) -> None:
        ...
        # 3. Model Training
        raw_model = self.model_agent.get_raw_model()
        if "IsolationForest" in str(type(raw_model)):
            self.model_agent.fit(X_features)
        else:
            # If the model is supervised (like XGBoost) but no labels are in the dataset,
            # fit it using pseudo-labels generated by a baseline Isolation Forest.
            # This allows surrogate XGBoost classification to enable tree SHAP.
            from sklearn.ensemble import IsolationForest
            iso = IsolationForest(contamination=0.01, random_state=42, n_jobs=-1)
            pseudo_labels = np.where(iso.fit_predict(X_features) == -1, 1, 0)
            self.model_agent.fit(X_features, pd.Series(pseudo_labels))
